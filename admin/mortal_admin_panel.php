        ...
        <div class="col-12">
            ...
            <div class="character-table-wrapper table-responsive rounded-3">
                <table class="character-table table table-dark table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php
                        $r = mysqli_query($conn, "SELECT m.*, u.username as owner_username FROM mortal_characters m LEFT JOIN users u ON m.user_id = u.id ORDER BY m.id DESC");
                        if (!$r) echo "<tr><td colspan='4'>" . htmlspecialchars(mysqli_error($conn)) . "</td></tr>";
                        elseif (mysqli_num_rows($r) === 0) echo "<tr><td colspan='4' class='text-center'>No Mortal characters found.</td></tr>";
                        else {

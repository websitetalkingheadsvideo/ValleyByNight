...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            ...
            <h1 class="mb-4">Demon Character Management</h1>
                ...
                <table class="character-table table table-dark table-hover align-middle mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center text-nowrap">House</th>
                            <th class="text-center text-nowrap">Faction</th>
                    ...
                    <tbody>
                        <?php
                        $r = mysqli_query($conn, "SELECT d.*, u.username as owner_username FROM demon_characters d LEFT JOIN users u ON d.user_id = u.id ORDER BY d.id DESC");
                        if (!$r) echo "<tr><td colspan='5'>" . htmlspecialchars(mysqli_error($conn)) . "</td></tr>";
                        elseif (mysqli_num_rows($r) === 0) echo "<tr><td colspan='5' class='text-center'>No Demon characters found.</td></tr>";
                        else {
                            while ($c = mysqli_fetch_assoc($r)) {
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><?php echo htmlspecialchars($c['house'] ?? '—'); ?></td>
                                    <td class="text-center"><?php echo htmlspecialchars($c['faction'] ?? '—'); ?></td>
...
<?php $apiEndpoint = '/admin/view_demon_character_api.php'; $modalId = 'viewCharacterModal'; include __DIR__ . '/../includes/character_view_modal.php'; ?>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(function(b) {
        b.addEventListener('click', function() {
            ...
            fetch('/admin/delete_demon_character_api.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ character_id: this.dataset.id }) })

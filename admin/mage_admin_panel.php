...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            ...
            <h1 class="mb-4">Mage Character Management</h1>
                ...
                <table class="character-table table table-dark table-hover align-middle mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center text-nowrap">Tradition</th>
                            <th class="text-center text-nowrap">Arete</th>
                    ...
                    <tbody>
                        <?php
                        $r = mysqli_query($conn, "SELECT m.*, u.username as owner_username FROM mage_characters m LEFT JOIN users u ON m.user_id = u.id ORDER BY m.id DESC");
                        if (!$r) echo "<tr><td colspan='5'>" . htmlspecialchars(mysqli_error($conn)) . "</td></tr>";
                        elseif (mysqli_num_rows($r) === 0) echo "<tr><td colspan='5' class='text-center'>No Mage characters found.</td></tr>";
                        else {
                            while ($c = mysqli_fetch_assoc($r)) {
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><?php echo htmlspecialchars($c['tradition'] ?? '—'); ?></td>
                                    <td class="text-center"><?php echo (int)($c['arete'] ?? 0); ?></td>
...
<?php $apiEndpoint = '/admin/view_mage_character_api.php'; $modalId = 'viewCharacterModal'; include __DIR__ . '/../includes/character_view_modal.php'; ?>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(function(b) {
        b.addEventListener('click', function() {
            ...
            fetch('/admin/delete_mage_character_api.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ character_id: this.dataset.id }) })

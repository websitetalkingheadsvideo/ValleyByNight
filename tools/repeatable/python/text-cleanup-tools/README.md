...
**Note:** These tools clean up text that has already been extracted via OCR. To actually perform OCR on image-based PDFs, use the OCR tools in `ocr-tools/`.
...
**Purpose:** Processes a folder of OCR-extracted text files to fix split words.
...
**Purpose:** Processes full OCR-extracted text files to fix split words.

<?php
/**
 * Demon Character JSON Import Script
 *
 * Imports Demon: The Fallen character JSON files from reference/Characters/Demon/
 * into the database. Upsert by character_name (update if exists, insert if new).
 *
 * Usage:
 *   CLI: php tools/repeatable/php/database-tools/import_demon_characters.php [filename.json]
...
if (!$is_cli) {
    ...
    echo "<!DOCTYPE html><html><head><title>Demon Character Import</title><style>body{font-family:monospace;padding:20px;} .success{color:green;} .error{color:red;} .warning{color:orange;}</style></head><body>";
}
...
function findDemonCharacterByName(mysqli $conn, string $character_name): ?int {
    $result = db_fetch_one($conn,
        "SELECT id FROM demon_characters WHERE character_name = ? LIMIT 1",
        's',
        [$character_name]
    );
...
function importDemonCharacter(mysqli $conn, array $data, string $filename): bool {
    global $stats;
    ...
    $character_id = findDemonCharacterByName($conn, $character_name);
    ...
    $cleanData = [
        ...
        'torment_permanent' => cleanInt($data['torment_permanent'] ?? 0),
        'torment_current' => cleanInt($data['torment_current'] ?? 0),
        'faith_permanent' => cleanInt($data['faith_permanent'] ?? 0),
        'faith_current' => cleanInt($data['faith_current'] ?? 0),
        'willpower_permanent' => cleanInt($data['willpower_permanent'] ?? 5),
        'willpower_current' => cleanInt($data['willpower_current'] ?? 5),
        'apocalyptic_form' => cleanJsonData($data['apocalyptic_form'] ?? null),
        'evocations' => cleanJsonData($data['evocations'] ?? null),
        'lores' => cleanJsonData($data['lores'] ?? null),
        ...
        'thralls_pacts' => cleanJsonData($data['thralls_pacts'] ?? null),
    ...
    try {
        $base_fields = [
            ...
            'nature', 'demeanor', 'concept', 'house', 'faction', 'cover',
            'pc', 'appearance', 'host_form_appearance', 'biography', 'notes', 'equipment', 'status',
            'torment_permanent', 'torment_current', 'faith_permanent', 'faith_current',
            'willpower_permanent', 'willpower_current',
            'apocalyptic_form', 'evocations', 'lores',
            'attributes', 'abilities', 'backgrounds', 'backgroundDetails',
            'merits_flaws', 'thralls_pacts', 'health_levels', 'relationships', 'custom_data',
        ...
        if ($character_id > 0) {
            ...
            $update_sql = "UPDATE demon_characters SET " . implode(', ', $set_parts) . " WHERE id = ?";
            $params = [];
            $types = '';
            foreach ($base_fields as $f) {
                if ($f === 'character_name') {
            ...
            $insert_sql = "INSERT INTO demon_characters ($field_list) VALUES ($placeholders)";
...
$demons_dir = __DIR__ . '/../../../reference/Characters/Demon/';

if ($import_all) {
    $files = glob($demons_dir . '*.json');
    $files = array_filter($files, function ($f) {
        return basename($f) !== 'demon_character_template.json';
    });

    if (empty($files)) {
        echo $is_cli ? "No JSON files found in $demons_dir\n" : "<p class='warning'>No JSON files found in Demon directory.</p>";
    } else {
        foreach ($files as $file) {
            ...
            if (importDemonCharacter($conn, $data, $filename)) {
    ...
    $file_path = $demons_dir . $file_param;
    ...
    die($is_cli ? "Usage: php import_demon_characters.php [filename.json]\n" : "<p class='error'>Usage: ?file=filename.json or ?all=1</p></body></html>");
}

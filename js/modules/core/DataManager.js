...
class DataManager {
    constructor() {
        ...
        this.maxRetries = 1; // Reduced from 3 - only retry once for network issues, not server errors
    ...
    shouldRetry(error, cacheKey) {
        ...
        // Don't retry on 500 errors - these are server-side issues that won't be fixed by retrying
    ...
    async saveCharacter(characterData) {
        try {
            const response = await this.fetchData('includes/save_character.php', {
                method: 'POST',
    ...
    async loadCharacter(characterId) {
        try {
            // Disable cache for character loading to ensure we always get fresh data
            // Each character should be loaded fresh, not from cache
            const data = await this.fetchData(`load_character.php?id=${characterId}`, {
                method: 'GET',
                useCache: false, // Disable cache to prevent loading wrong character
                cacheTimeout: 0
            });

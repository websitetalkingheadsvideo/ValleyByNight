...
This tracking system scans all NPCs in the VbN data store and compares them against existing Character Teaser files to identify which NPCs are missing Character Teasers.

**Goal**: Ensure every NPC in the database has a corresponding Character Teaser file.

## What It Does

1. **Queries Character Data**: Retrieves all NPCs from the `characters` dataset where `player_name = 'NPC'`
...
**Note**: If any field is missing in the source data, it will be set to `null` in the JSON output.
...
- Running the script does not modify the source data or existing Character Teaser files
...
- PHP 7.4+ (or PHP 8.x)
- Supabase environment configured for the project
...
- Verify the project's Supabase environment is configured correctly
- Check that the `characters` table is accessible through the shared client
...
- **After adding new NPCs** to the project data
...
- The script does not modify the source data or existing files

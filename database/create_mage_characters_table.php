<?php
/**
 * Database Migration: Create mage_characters table
 *
 * Creates a separate table for Mage: The Ascension characters without affecting
 * the existing VtM, wraith, werewolf, or mortal character tables.
 *
 * Run via browser: database/create_mage_characters_table.php
 * Or via CLI: php database/create_mage_characters_table.php
 */
...
$check_table = "SHOW TABLES LIKE 'mage_characters'";
$result = mysqli_query($conn, $check_table);

if ($result && mysqli_num_rows($result) > 0) {
    echo "<h2>Table 'mage_characters' already exists.</h2>";
    echo "<p>If you want to recreate it, drop it first:</p>";
    echo "<pre>DROP TABLE IF EXISTS mage_characters;</pre>";
    mysqli_free_result($result);
    exit;
}

$create_table_sql = "
CREATE TABLE IF NOT EXISTS mage_characters (
...
if (mysqli_query($conn, $create_table_sql)) {
    echo "<h2>Success: Table 'mage_characters' created successfully.</h2>";
    echo "<p>The table includes columns for Mage: The Ascension (tradition, paradigm, spheres, arete, quintessence, paradox, rotes, etc.).</p>";

    echo "<h3>Table Structure:</h3>";
    $describe_sql = "DESCRIBE mage_characters";

<?php
/**
 * Update Dorikhan Caine Image Script
 */
...
$character_name = 'Dorikhan Caine';
$image_filename = 'Dorikhan Caine.png';

if (!$is_cli) {
    echo "<!DOCTYPE html><html><head><title>Dorikhan Image Update</title></head><body>";
    echo "<h1>Dorikhan Caine Image Update</h1>";
    echo "<pre>";
}
...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!file_exists($image_path)) {
        echo "Error: Image file not found: $image_filename\n";
        ...
        if ($update_stmt) {
            ...
            if (mysqli_stmt_execute($update_stmt)) {
                ...
                echo "Previous value: " . ($current_image ?: '(empty)') . "\n";
            } else {
                echo "Error: Failed to update: " . mysqli_stmt_error($update_stmt) . "\n";
            }
            
            mysqli_stmt_close($update_stmt);
        } else {
            echo "Error: Failed to prepare update statement: " . mysqli_error($conn) . "\n";
        }
    }
} else {
